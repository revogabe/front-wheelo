...
export default function ModalEditFuel({
  carId,
  typeFuel,
  quantity,
  price,
  id,
}: CarIdProps & FuelValue) {
  ...
  const createFuel = useMutation({
    mutationFn: (data: FormFuelProps) => {
      return api.patch(`/supply/update/${carId}`, data, {
        data: {
          id,
  ...
  return (
    <div className="h-full w-full">
      ...
      />
      <ButtonUnstyled
        className="bg-gray-3 border border-gray-8 py-2 px-4 text-gray-11 rounded hover:bg-gray-6 hover:border-gray-10 cursor-pointer hover:text-gray-11 transition-all"
        onClick={handleOpen}
      >
        Editar
      </ButtonUnstyled>
      ...
      >
        <Box className="w-full max-w-md  m-auto p-8 gap-2 rounded-lg shadow-black/10 shadow-xl flex flex-col h-max bg-gray-1">
          ...
          >
            ...
            <input
              type="number"
              {...register('price', { valueAsNumber: true, value: price })}
              className="bg-gray-1 border border-gray-7 w-full px-4 py-4 focus:outline-orange-11 rounded-md focus:border-transparent "
              placeholder="Preço (R$)"
            />
